...
class TextFieldsDataGrid extends StatelessWidget {
  final List<DataQualityProfile> profiles;

  const TextFieldsDataGrid({super.key, required this.profiles});
  ...
  Widget build(BuildContext context) {
    return SfDataGridTheme(
      ...
      child: SfDataGrid(
        ...
        headerRowHeight: 30,
        source: TextFieldsDataSource(context, profiles),
        columns: <GridColumn>[
          GridColumn(
          ...
          GridColumn(
            columnName: 'distinct',
            label: _buildColumnLabel(context, 'Distinct'),
          ),
          GridColumn(
            columnName: 'duplicates',
            label: _buildColumnLabel(context, 'Duplicates'),
          ),
          GridColumn(
            columnName: 'nonCompliant',
            label: _buildColumnLabel(context, 'Non-Compliant'),
          ),
        ],
      ),
    );
...
class TextFieldsDataSource extends DataGridSource {
  final BuildContext context;
  List<DataGridRow> dataGridRows = [];

  TextFieldsDataSource(this.context, List<DataQualityProfile> profiles) {
    dataGridRows = profiles.map<DataGridRow>((profile) {
      return DataGridRow(cells: [
        ...
        DataGridCell<int>(columnName: 'distinct', value: profile.distinctCount),
        DataGridCell<int>(columnName: 'duplicates', value: profile.duplicateCount),
        DataGridCell<int>(columnName: 'nonCompliant', value: profile.nonCompliantCount),
      ]);
    }).toList();
  }
